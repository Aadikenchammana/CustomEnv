...
def create_parallel_sim(self):
    parallel_sim = simfire.sim.simulation.FireSimulation(self.config)
    num_updates = 0
    while parallel_sim.fire_status == GameStatus.RUNNING and num_updates <= int(self.episode_steps/self.simulation_steps_per_timestep):
        for item in self.mitigations_log[num_updates]:
            parallel_sim.update_mitigation([(item[0],item[1],BurnStatus.FIRELINE)])
        if num_updates < int(self.episode_steps/self.simulation_steps_per_timestep):
            parallel_sim.fire_sprites = parallel_sim.fire_manager.sprites
            parallel_sim.fire_map, parallel_sim.fire_status = parallel_sim.fire_manager.update(parallel_sim.fire_map)
            parallel_sim.elapsed_steps += 1
        num_updates += 1
        
    return parallel_sim

def run_one_parallel_step(sim,steps):
    num_updates = 0
    while num_updates < steps:
    ...
    return sim.fire_map, sim.active, sim

def generate_real_probabilities(self, steps):
    past_map = self.fire_map
    prob_map = np.zeros_like(past_map)
    prob_map = prob_map.astype(np.float32)
    parallel_sim = create_parallel_sim(self)
    for step in range(steps):
        new_map, burn_status, parallel_sim =  run_one_parallel_step(parallel_sim,1)
        prob_map[(past_map == 0) & (new_map == 1)] = 1/(step+1)
...
class CustomEnv(gym.Env):
    ...
    def __init__(self):
        ...
        print(self.config)
        self.preset_fires_starts = [(15,5)]#[(5,5),(22,5),(22,22),(5,22),(13,13),(5,13),(13,5), (22,13), (13, 22)]
        ...
        self.episode_num = 0
        self.autoplace = True
        self.mitigations_log = {}
        self.recent_burnt = get_burned(self.fire_map)+get_burning(self.fire_map)
    ...
    def step(self, action):
        ...
        if self.autoplace and placement_fn(self):
            self.sim.update_mitigation([(self.agent_x,self.agent_y,BurnStatus.FIRELINE)])
            if int (self.episode_steps/self.simulation_steps_per_timestep) not in self.mitigations_log.keys():
                self.mitigations_log[int(self.episode_steps/self.simulation_steps_per_timestep)] = [[self.agent_x, self.agent_y]]
            else:
                self.mitigations_log[int(self.episode_steps/self.simulation_steps_per_timestep)] = self.mitigations_log[int(self.episode_steps/self.simulation_steps_per_timestep)]+[[self.agent_x, self.agent_y]]
        ...
        #real_prob_map = generate_real_probabilities(self,5)
        ...
        if self.chkpt_flag:
            ...
            np.save(self.chkpt_dir+"//"+str(self.episode_steps)+"_prob.npy",self.prob_map)
            agent_map = copy.deepcopy(self.fire_map)
            agent_map[self.agent_y][self.agent_x] = 4
            np.save(self.chkpt_dir+"//"+str(self.episode_steps)+"_agent.npy",agent_map)
            #np.save(self.chkpt_dir+"//"+str(self.episode_steps)+"_realprob.npy",real_prob_map)
        if self.episode_steps%self.simulation_steps_per_timestep == 0:
    ...
    def reset(self, seed=None, options=None):
        ...
        print("----------------")
        if self.recent_burnt < 170:
            print("FUUUUUCCCKKKKKKKK TEEEEEEAAAAAa")
        elif self.recent_burnt < 200:
            print("WEEEE GETTTTTING THERRRREE")
        print(self.recent_burnt)
        print("----------------")
...
model_path = 'previous_eval_models//pretrainedSlinkTest.zip'
model = PPO.load(model_path, env=env)
save_path = 'saved_models//'+datetime.now().strftime("%m.%d.%Y_%H:%M:%S")
os.mkdir(save_path)
save_path += "//"
callback = SaveModelCallback(save_path=save_path, check_freq=100)
