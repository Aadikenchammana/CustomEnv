...
class SaveModelCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.check_freq == 0:
            print(os.path.join(self.save_path, f'PBPR{self.num_timesteps}'))
            self.model.save(os.path.join(self.save_path, f'PBPR_{self.num_timesteps}'))
        return True
...
class CustomEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.sim = simfire.sim.simulation.FireSimulation(self.config)
        self.sim.reset()
        self.fire_map, self.fire_status = run_one_simulation_step(self, 4)
