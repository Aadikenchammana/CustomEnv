...
def run_one_simulation_step(self, total_updates):
    ...
    self.fire_map = scale_array(self.sim.fire_map, self.scaled_screen_size)
    return self.fire_map, self.sim.active
...
def scale_array(arr, final_size):
    #t = time.time()
    ...
    for i in range(final_size):
        for j in range(final_size):
            ...
            region_mode = np.sum(region)/(start_x-end_x)**2#mode(region, axis=None), changed from mode for time conscerns
            scaled_array[i, j] = region_mode if region_mode.size > 0 else 0
    #print("TIMONG1:",t-time.time())  
    return scaled_array


def change_scaled_region(original_array, scaled_array, scaled_x, scaled_y, new_value):
    #t = time.time()
    factor = original_array.shape[0] / scaled_array.shape[0]
    scaled_array[scaled_x, scaled_y] = new_value
    start_x = int(scaled_x * factor)
    end_x = int((scaled_x + 1) * factor)
    start_y = int(scaled_y * factor)
    end_y = int((scaled_y + 1) * factor)
    original_array[start_x:end_x, start_y:end_y] = new_value
    changed_coords = []
    for x in range(start_x, end_x):
        for y in range(start_y, end_y):
            changed_coords.append((x, y))
    #print("TIMONG:",t-time.time())
    return original_array, scaled_array, changed_coords

def update_scaled_mitigations(self, typ):
    unscaled_map, scaled_map, changed_points = change_scaled_region(self.sim.fire_map, self.fire_map, self.agent_x, self.agent_y, 3)
    new_mitigations = [(item[0],item[1],typ) for item in changed_points]
    self.sim.update_mitigation(new_mitigations)
    return scaled_map
...
class SaveModelCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.check_freq == 0:
            print(os.path.join(self.save_path, f'PBPS_{self.num_timesteps}'))
            self.model.save(os.path.join(self.save_path, f'PBPS_{self.num_timesteps}'))
        return True
...
class CustomEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.simulation_steps_per_timestep = 6
        self.episode_num = 0
        self.autoplace = True
        ...
        n_channel = 2
        self.observation_space = spaces.Box(low=0, high=4,shape=(n_channel, self.scaled_screen_size, self.scaled_screen_size), dtype=np.float32)
    ...
    def step(self, action):
        ...
        if self.agent_x > self.scaled_screen_size-1:
            self.agent_x = self.scaled_screen_size-1 #self.agent_start[0]
        if self.agent_y > self.scaled_screen_size-1:
            self.agent_y = self.scaled_screen_size-1 #self.agent_start[1]
        if self.agent_x < 0:
        ...
        if action_str == "fireline":
            update_scaled_mitigations(self, BurnStatus.FIRELINE)
        ...
        if self.episode_steps > self.total_steps_per_episode:
            terminated = True
        if get_burning(self.sim.fire_map) == 0 or not self.fire_status:
            terminated = True
            truncated = False
