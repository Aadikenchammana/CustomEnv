...
class SaveModelCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.check_freq == 0:
            print(os.path.join(self.save_path, f'BPBOG_{self.num_timesteps}'))
            self.model.save(os.path.join(self.save_path, f'BPBOG_{self.num_timesteps}'))
        return True
...
class CustomEnv(gym.Env):
    ...
    def __init__(self):
        ...
        n_channel = 2
        self.observation_space = spaces.Box(low=0, high=4,shape=(n_channel, self.screen_size, self.screen_size), dtype=np.int64)
